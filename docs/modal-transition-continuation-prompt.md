...
**M2 (CF data hosting) — code complete, pending seed + E2E:**
...
**Seeding script:** `scripts/seed_r2.py` walks local `data/`, uploads to R2. Skips intermediates/, .npz, .pkl in embeddings/, layer_cache/. Dry-run by default, `--execute` to upload.
...
**M2 remaining:**
1. Run `scripts/seed_r2.py --execute` to populate R2 bucket
2. E2E test: delete local data/, start app, verify R2 download works
3. Commit + PR
